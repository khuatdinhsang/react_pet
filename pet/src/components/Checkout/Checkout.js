...
import "./checkout.css"
function Checkout({ user, totalMoney, onBuyOrder }) {
    ...
    const handlePhoneNumber = (data) => {
        ...
        setCheckPhone((data) => !data)
    }
    const buyOrder = () => {
        alert("hang cua ban se dc chuyen den trong 2 ngay nua")
        onBuyOrder(true)
        navigate("/")
    }
    return (
            ...
            <div className="iphone">
                ...
                <div className="form" >
                    <div>
                        ...
                        <button
                            style={{ fontSize: 10, margin: 10, background: 'red', color: 'white' }
                            }
                            onClick={() => handlePhoneNumber(true)}
                        > Them SDT</button>
                        {checkPhone && <input type="text"
                            value={phoneNumber}
                    ...
                    <div>
                        <h2>Shopping Bill</h2>
                        {!isDiscount && <h3>Total: {totalMoney} VND</h3>}
                        <p style={{ color: 'red' }}>Nhap " sangdeptrai " sẽ được giảm giá</p>
                        <input style={{ padding: 10, background: 'red', color: 'white' }}
                            type="text"
                            value={discount}
                            onChange={(e) => {
                                setDiscount(e.target.value)
                                if (e.target.value === "sangdeptrai") {
                                    setIsDiscount(true)
                                } else {
                                    setIsDiscount(false)
                                }
                        ...
                        />
                        <br />
                        {isDiscount && <h3>Total:{totalMoney * 0.5} VND</h3>}
                    </div>
                    <div>
                        <button className="button button--full" onClick={() => buyOrder()}  ><svg className="icon">
